...
import {
  CalendarScreen,
  type CalendarPlanBuildContext,
} from "@/components/calendar/calendar-screen";
...
} from "@/components/method-runtime-panel";
...
import { calendarPersonalizationReasons } from "@/lib/calendar/model";
...
import { consolidatePersonalizationStateForCanonicalV1 } from "@/lib/personalization/canonical-profile-migration";
...
export function YovaPrototype({
  ...
  const [guidedSessionAllowance, setGuidedSessionAllowance] = useState<GuidedSessionAllowanceDisplayState>({
    kind: "unavailable",
    remainingToday: null,
  ...
  const sessionActivityProgressRef = useRef<SessionActivityProgress | null>(null);
  ...
  const recoverableSessionCheckpoints = activeSessionCheckpoints.filter((checkpoint) => {
    ...
    if (!plan || session?.status !== "ready") return false;
    if (checkpoint.methodWork && !checkpoint.resourceGeneratedAt) {
      const methodContext = recoveryMethodContext({
        ...
        adjustment: checkpoint.sessionAdjustment ?? null,
      });
  ...
  useEffect(() => {
    setAnswers((currentAnswers) => {
      const currentState = readPersonalizationStateFromAnswers(currentAnswers);
      if (!currentState.controls.experiments && !currentState.activeExperiment) {
        return currentAnswers;
      }
      return updatePersonalizationStateInAnswers(
        currentAnswers,
  ...
  const effectivePreviewPreferredMethodIds = personalizationState.controls.selfReport
    ...
    : [];
  const effectivePreviewCanonicalProfile = personalizationState.controls.selfReport
    ? canonicalLearnerProfileFromAnswers(answers)
    : null;
  const calendarReasons = calendarPersonalizationReasons({
    answers,
    completions: sessionCompletions,
    interruptions: sessionInterruptions,
  });
  const calendarProfileSummary = effectivePreviewCanonicalProfile
    ? buildCanonicalLearnerFacingSummary(effectivePreviewCanonicalProfile).statements
    : [];
  ...
  const personalizationWorkspaceClassName = workspaceClassName(
    canonicalProfileWorkspaceSettings({
      profile: personalizationState.controls.selfReport
        ? canonicalLearnerProfileFromAnswers(answers)
        : createCanonicalLearnerProfile([]),
      base: effectivePersonalizationWorkspaceSettings(
  ...
  const updateSessionActivityProgress = useCallback((progress: SessionActivityProgress | null) => {
    sessionActivityProgressRef.current = progress;
    setSessionActivityProgress(progress);
  }, []);
  ...
  const beginPlanCreation = () => {
    ...
    setCreatorCalendarEventId(null);
    setCreatorReviewSourceFirst(false);
    setStage("plan-creator");
  };

  const beginCalendarAdd = (seed?: AddIntakeSeed, context?: CalendarPlanBuildContext) => {
    setCreatorSeed(seed ?? null);
    setCreatorMilestoneId(null);
  ...
  const syncCheckpointToAccount = useCallback(async (checkpoint: ActiveSessionCheckpoint) => {
    ...
    // and built-in lesson checkpoints continue to sync as before.
    if (checkpoint.methodWork) return null;
    // An explicit Exit closes the old attempt before the fresh recovery run
    // may be written to the same cloud session. Keep the new checkpoint local
    // until that ordered terminal handoff has reached the server.
    if (loadQueuedSessionInterruptions(checkpoint.accountId).some((entry) => (
      entry.interruption.planSessionId === checkpoint.planSessionId
    ))) return null;
    if (
  ...
  const refreshGuidedSessionAllowance = useCallback(async (signal?: AbortSignal) => {
    try {
      const response = await fetch("/api/sessions/allowance", {
        method: "GET",
        cache: "no-store",
        signal,
      });
      const body: unknown = await response.json().catch(() => null);
      const next = guidedSessionAllowanceStateFromResponse(response, body);
      if (!signal?.aborted) {
        // A temporary status-read failure must not erase a known limit and
        // reopen a setup path that the server will still reject.
        setGuidedSessionAllowance((current) => (
          next.kind === "unavailable" && current.kind !== "unavailable"
            ? current
            : next
        ));
      }
  ...
  }, [ready, refreshGuidedSessionAllowance, signedIn, stage]);

  useEffect(() => {
    if (
      !signedIn
      || guidedSessionAllowance.kind === "available"
      || guidedSessionAllowance.kind === "unavailable"
    ) return;
    let cancelled = false;
    let timeoutId: number;
    const checkAfterReset = async () => {
      const next = await refreshGuidedSessionAllowance();
      if (cancelled) return;
      // Preserve the known server limit on an unavailable read, but keep
      // checking so a transient status outage cannot leave Home locked after
      // the real server reset has passed.
      if (!next || next.kind === "unavailable") {
        timeoutId = window.setTimeout(() => void checkAfterReset(), 30_000);
      }
    };
    timeoutId = window.setTimeout(
      () => void checkAfterReset(),
      guidedSessionAllowance.retryAfterSeconds * 1_000 + 250,
    );
    return () => {
      cancelled = true;
      window.clearTimeout(timeoutId);
    };
  }, [guidedSessionAllowance, refreshGuidedSessionAllowance, signedIn]);
    ...
    async function openYova() {
      ...
      if (cloudAccount) {
        ...
        try {
          ...
          setCalendarMaterials(cloudState.calendarMaterials ?? []);
  ...
  useEffect(() => {
    ...
    const invalidCheckpoints = activeSessionCheckpoints.filter((checkpoint) => {
      ...
      if (protectedCloudCompletion) return false;
      if (plan?.status !== "active" || !session || session.status !== "ready") return true;
      if (checkpoint.methodWork && !checkpoint.resourceGeneratedAt) {
        const methodContext = recoveryMethodContext({
          plan,
          session,
  ...
  }, [ready, account, stage, plans, answers, sessionCompletions, sessionInterruptions, activeSessionCheckpoints]);
  ...
  writeActiveSessionCheckpointRef.current = (
    ...
    const methodSession = standaloneMethodWork
      ? sessionRecoverySession
      : activePlan.studyMode === "outside_yova"
        ? {
          ...currentSession,
          learningMode: sessionMethodBriefing?.learningMode ?? currentSession.learningMode,
        }
        : null;
    ...
    const boundedMethodProgress = methodSession
      ? boundedMethodWorkProgress(methodWorkProgress, methodTopics)
      : null;
    const checkpointMethodWork = boundedMethodProgress && (
      standaloneMethodWork
      || boundedMethodProgress.checkedTopics.length > 0
      || boundedMethodProgress.sourceReviewed
    )
      ? boundedMethodProgress
      : null;
    const resourceFingerprint = activeSessionResourceFingerprintRef.current
      ?? (standaloneMethodWork && methodSession
        ? fingerprintMethodWorkSession({
        ...
        : currentSession.resource
          ? fingerprintSessionResource(currentSession.resource)
          : null);
    const resourceGeneratedAt = standaloneMethodWork
      ? null
      : activeSessionResourceGeneratedAtRef.current
        ?? currentSession.resource?.generatedAt
        ?? null;
    if (!resourceFingerprint || (!standaloneMethodWork && !resourceGeneratedAt)) return false;
    ...
    const methodCounts = standaloneMethodWork && checkpointMethodWork
      ? methodWorkCheckpointCounts({
    ...
    const checkpoint: ActiveSessionCheckpoint = awaitingFinish
      ? {
        ...
        completedSteps: checkpointCompletedSteps,
        totalSteps: checkpointTotalSteps,
        resumeStep: checkpointResumeStep,
        evidence,
        resourceFingerprint,
        ...(resourceGeneratedAt ? { resourceGeneratedAt } : {}),
  ...
  const startSession = async (
    ...
    const storedRequestedSession = planSessionId
      ? storedRequestedPlan.sessions.find((session) => (
        session.id === planSessionId && session.status === "ready"
      ))
  ...
  const requestSessionStart = (planId?: string, planSessionId?: string) => {
    const requestedPlan = planId
      ...
      : activePlan;
    const readySessions = requestedPlan?.sessions.filter((session) => session.status === "ready") ?? [];
    const requestedSession = planSessionId
      ? readySessions.find((session) => session.id === planSessionId) ?? null
      : readySessions[0] ?? null;
    if (!requestedPlan || !requestedSession) return false;
    if (planSessionId && readySessions.length !== 1) return false;
    const startDecision = sessionStartRecoveryDecision({
      plan: requestedPlan,
      session: requestedSession,
      interruptions: sessionInterruptions,
    ...
    if (!startDecision.advertiseContinue && isSessionAheadOfSchedule(requestedSession)) {
      setEarlySessionPlanId(requestedPlan.id);
      setEarlySessionPlanSessionId(requestedSession.id);
  ...
  const startEarlySession = async (shiftRemainingPlan: boolean) => {
    ...
    if (!requestedPlan || !requestedSession || earlySchedulePending) return;
    ...
    try {
      ...
      const authoritativeUpdates = account?.identityMode === "supabase"
        ? (await persistPlanSchedule(requestedPlan.id, updates, { operationKind: "advance_now" })).sessions
        : updates;
      const advancedPlan = applyAdvancedSchedule(requestedPlan, authoritativeUpdates);
      ...
      if (typeof window !== "undefined") {
        ...
        const firstUpdate = authoritativeUpdates[0];
        const priorSession = firstUpdate
        ...
        if (firstUpdate && priorSession) {
          const logged = await appendCalendarChangeLogEntry(window.localStorage, calendarAccountId, {
            ...
            reason: "You approved Start and adjust calendar. YOVA preserved the saved plan order while moving the remaining schedule forward.",
            origin: "automatic",
            undoable: false,
            undoneAt: null,
            undo: {
              ...
              planId: requestedPlan.id,
              planSessionId: priorSession.id,
              from: priorSession.scheduledFor,
              to: firstUpdate.scheduledFor,
            },
          });
          if (logged) setCalendarStorageRevision((current) => current + 1);
          else setCloudSyncIssue("The plan was moved forward, but its Calendar history could not be saved on this device.");
        }
      }
      setEarlySessionPlanId(null);
      setEarlySessionPlanSessionId(null);
  ...
  const completeActiveSession = async (correctAnswers: number, totalAnswers: number, feedback: SessionCompletion["feedback"], actualMinutes: number, applyRecommendedChange: boolean) => {
    ...
    const completion = sessionCompletionMode === "unguided_practice"
      ...
      : completionDraft;
    const nextSession = nextUnfinishedSessionAfter(activePlan.sessions, currentSession.sequence);
    const deferredContinuationRequired = sessionCompletionMode === "guided"
      && sessionResourceHasDeferredPlanTargets(currentSession);
    const deferredContinuation = deferredContinuationRequired
      && activePlan.sessions.length < MAX_RUNTIME_PLAN_SESSIONS
      ? buildDeferredSessionContinuation({
        completedSession: currentSession,
        completedAt: completion.completedAt,
        plannedMinutes: completion.plannedMinutes,
        continuationId: makeUuid(),
        nextUnfinishedSession: nextSession,
    ...
    const decision = sessionCompletionMode === "guided" && !deferredContinuation
      ? buildPostSessionDecision(currentSession, nextSession, completion)
      : null;
    const adaptationAgencyMode = decision?.kind === "adapt_next_session"
      && nextSession?.studyRoute?.identity.lifecycleStatus === "committed"
      ? agencyModeForStudyRouteControlMode(nextSession.studyRoute.agency.controlMode).mode
      : null;
    const shouldApplyRecommendedChange = applyRecommendedChange
      || adaptationAgencyMode === "yova_decides";
    const approvedChanges = decision
      ? approvedPostSessionChanges(decision, shouldApplyRecommendedChange)
      : { adaptation: null, followUpSession: null };
    const requestedAdaptation = approvedChanges.adaptation;
    ...
    try {
      const transitionInput = {
        ...
        adaptation: requestedAdaptation,
        followUpSession: delayedVerification,
        continuationSession: deferredContinuation,
      };
      if (!requestedAdaptation || !adaptationAgencyMode) {
        routeTransition = preparePostSessionStudyRouteTransition(transitionInput);
      } else {
        const proposedTransition = preparePostSessionStudyRouteTransition({
          ...transitionInput,
          adaptationAgency: {
        ...
        const proposedDecision = proposedTransition.adaptationAgencyDecision;
        if (adaptationAgencyMode === "help_me_choose") {
          const candidateRoute = proposedDecision?.candidateRoute;
          const requiredConfirmation = proposedDecision?.requiredConfirmation;
          if (
            proposedDecision?.status !== "confirmation_required"
            || !candidateRoute
            || !requiredConfirmation
          ) {
            throw new Error("The exact post-session route confirmation was not prepared.");
          }
          routeTransition = preparePostSessionStudyRouteTransition({
            ...
            adaptationAgency: {
              ...
              confirmation: {
                ...requiredConfirmation,
                confirmedAt: completion.completedAt,
              },
            },
          });
        } else if (adaptationAgencyMode === "ill_customize") {
          const candidateRoute = proposedDecision?.candidateRoute;
          if (proposedDecision?.status !== "recommendation_available" || !candidateRoute) {
            throw new Error("The learner-controlled route recommendation was not prepared.");
          }
          routeTransition = preparePostSessionStudyRouteTransition({
            ...transitionInput,
            adaptationAgency: {
          ...
          routeTransition = proposedTransition;
        }
      }
    } catch {
    ...
    const adaptation = routeTransition.adaptationAgencyDecision === undefined
      ? requestedAdaptation
      : routeTransition.appliedAdaptation ?? null;
    if (requestedAdaptation && adaptationAgencyMode && !adaptation) {
      setSessionRecoveryIssue("YOVA kept this session open because the approved route change was not committed exactly. Try finishing again; your current work is still saved.");
      return false;
    }
    ...
    trackProductEvent({
      ...
      context: {
        ...
        adaptedNextSession: adaptation !== null || delayedVerification !== null || deferredContinuation !== null,
  ...
  const interruptActiveSession = () => {
    ...
    const terminalCheckpoint = exitCheckpointSaved && checkpointRunId && account
      ...
      : null;
    const terminalCheckpointSavedAt = terminalCheckpoint
      ? Date.parse(terminalCheckpoint.savedAt)
      : Number.NaN;
    const exitRecoveryCheckpoint = terminalCheckpoint && Number.isFinite(terminalCheckpointSavedAt)
      ? handoffActiveSessionCheckpointAfterExit(
        terminalCheckpoint,
        interruption,
        makeUuid(),
        new Date(Math.max(
          Date.now(),
          interruptedAt.getTime() + 1,
          terminalCheckpointSavedAt + 1,
        )).toISOString(),
      )
      : null;
    const exitRecoverySaved = Boolean(
      exitRecoveryCheckpoint
      && saveActiveSessionCheckpoint(exitRecoveryCheckpoint),
    );
    if (exitRecoveryCheckpoint && exitRecoverySaved) {
      setActiveSessionCheckpoints((current) => [
        ...current.filter((checkpoint) => !(
          checkpoint.accountId === exitRecoveryCheckpoint.accountId
          && checkpoint.planSessionId === exitRecoveryCheckpoint.planSessionId
        )),
        exitRecoveryCheckpoint,
      ]);
    }
    ...
    if (account?.identityMode === "supabase") {
      ...
      void interruptionSync
        .then(async (synced) => {
          ...
          const latestRecoveryCheckpoint = exitRecoveryCheckpoint && exitRecoverySaved
            ? loadActiveSessionCheckpoints(account.id).find((checkpoint) => (
              checkpoint.planSessionId === currentSession.id
              && checkpoint.runId === exitRecoveryCheckpoint.runId
            )) ?? null
            : null;
          const recoverySyncIssue = latestRecoveryCheckpoint
  ...
  const resetYovaData = async () => {
    ...
    if (account) {
      clearActiveSessionCheckpoints(account.id);
      if (typeof window !== "undefined") {
        clearCalendarPrototypeState(window.localStorage, account.id);
      }
    }
    checkpointSyncEpochRef.current += 1;
    discardedCheckpointRunIdsRef.current.clear();
    ...
    if (isCloudAccount) {
      ...
      setGuidedSessionAllowanceChecking(true);
      setStage("landing");
    }
  };

  const rescheduleSessions = (planId: string, updates: readonly ScheduleSessionUpdate[]) => {
    setPlans((current) => current.map((plan) => (
      plan.id === planId ? applyAdvancedSchedule(plan, updates) : plan
    )));
  };
  ...
  const preserveSeedDeadline = (plan: LearningPlan) => {
    ...
    const calendarEventId = creatorCalendarEventId;
    setCreatorSeed(null);
    setCreatorMilestoneId(null);
    setCreatorCalendarEventId(null);
    if (milestoneId) {
    ...
    void (async () => {
      try {
        ...
        if (calendarEventId && typeof window !== "undefined") {
          ...
          if (!removed) {
            setCloudSyncIssue("The plan and linked deadline were saved, but the earlier device-only Calendar item could not be removed. Delete that duplicate item manually.");
          } else {
            setCalendarStorageRevision((current) => current + 1);
          }
        }
      } catch (error) {
        setCloudSyncIssue(error instanceof Error
          ? error.message
          : "The learning plan was saved, but its deadline still needs to be added to Calendar.");
      }
    })();
  };
  ...
  const signOut = async ({ accountAlreadyDeleted = false }: { accountAlreadyDeleted?: boolean } = {}) => {
    ...
    try {
      const { fullyCleared } = clearConfirmedSignOutStorage(signingOutAccountId, {
        clearDeletedAccountCalendar: accountAlreadyDeleted,
      });
  ...
  const beginRecoveryMethodPractice = () => {
    ...
    const topics = methodPracticeTopics(sessionRecoverySession, null);
    const sourceFirstRequired = activePlan.studyMode === "outside_yova"
      && sessionRecoverySession.learningMode === "learn";
    activeSessionResourceFingerprintRef.current = fingerprintMethodWorkSession({
      studyMode: activePlan.studyMode,
      session: sessionRecoverySession,
      topics,
      sourceFirstRequired,
    });
    activeSessionResourceGeneratedAtRef.current = null;
  ...
  const advanceActiveSession = async (evaluation: AnswerEvaluationResponse | null) => {
    ...
    if (
      ...
      && currentActivity.methodPhase !== "pretest"
  ...
  if (stage === "onboarding") {
    ...
    return (
      <OnboardingQuestion
        index={questionIndex}
        answer={canonicalProfileSignal(
          onboardingProfile,
          onboardingQuestion.signalId,
        )?.value}
        onBack={() => setQuestionIndex((value) => Math.max(0, value - 1))}
        onAnswer={(answer) => {
          setAnswers(writeCanonicalLearnerProfileToAnswers(
            answers,
            canonicalProfileWithQuestionAnswer(
              onboardingProfile,
        ...
        onNext={() => {
          if (questionIndex === CANONICAL_PROFILE_QUESTIONS.length - 1) {
            trackProductEvent({
              eventName: "onboarding_completed",
              context: { answeredQuestionCount: onboardingProfile.signals.length },
            }, analyticsEnabled);
  ...
  if (stage === "session-method") return <StandaloneStudyMethodSession
    ...
    onProgressChange={setMethodWorkProgress}
  ...
  if (stage === "session") {
    return (
      <GuidedSession
        ...
        methodWorkProgress={methodWorkProgress}
        activityProgress={sessionActivityProgress}
        onMethodWorkProgressChange={setMethodWorkProgress}
        onActivityProgressChange={(progress) => updateSessionActivityProgress(progress)}
  ...
  const revisionClient: RevisionClient = {
    ...
    profileSummary: buildPlanProfileSummary(answers), previewCanonicalProfile: effectivePreviewCanonicalProfile,
  ...
  return <>
    <AppShell activeTab={activeTab} onTab={openTab} account={account} cloudSyncIssue={cloudSyncIssue} signOutIssue={signOutIssue} signingOut={signingOut} onRetryCloudSync={retryCloudSync} onAdd={() => beginCalendarAdd()} workspaceClassName={personalizationWorkspaceClassName} onSignOut={signOut}>
      ...
      {activeTab === "Calendar" && <CalendarScreen
        ...
        milestones={agendaMilestones}
        sessionCompletions={sessionCompletions}
        sessionInterruptions={sessionInterruptions}
        activeSessionCheckpoints={recoverableSessionCheckpoints}
        adjustmentProtectedSessionIds={activeSessionCheckpoints.map((checkpoint) => checkpoint.planSessionId)}
        calendarMaterials={calendarMaterials}
        personalizationReasons={calendarReasons}
        personalizationSummary={calendarProfileSummary}
        allowance={guidedSessionAllowance}
        allowanceChecking={guidedSessionAllowanceChecking}
        previewMode={browserPreviewMode || account?.identityMode === "preview"}
        onOpenAdd={beginCalendarAdd}
        onOpenPlan={(planId) => {
          setSelectedPlanId(planId);
          setLearningDetailPlanId(planId);
          setActiveTab("Learning");
        }}
        onStart={(target) => requestSessionStart(target.planId, target.planSessionId)}
        onActivateReview={activateConceptReview}
        onReschedule={rescheduleSessions}
        onAdjustDuration={adjustSessionDuration}
        onClassifyRecoveryInterruption={setRecoveryEvidenceClassification}
        onUpdateMilestone={updateDeadlineMilestone}
        onDeleteMilestone={deleteDeadlineMilestone}
        onAskAdjust={() => {
          setTutorQuestion("Help me adjust this week's calendar around the time I actually have without losing required work.");
          openAskYova();
        }}
        onConvertMilestone={(milestone, outcome) => {
          setCreatorSeed({
            title: milestone.title,
            objective: milestone.description || `Complete ${milestone.title}`,
            itemType: /\b(exam|test|quiz|midterm|final)\b/i.test(milestone.title)
              ? "test"
              : "assignment",
            dueAt: milestone.dueAt,
            scope: milestone.description || milestone.title,
            progress: "",
            materialsSummary: "No materials attached yet.",
            missingFields: milestone.description ? [] : ["scope"],
            description: milestone.description || milestone.title,
            materials: [],
          });
          setCreatorMilestoneId(milestone.id);
          setCreatorCalendarEventId(null);
          setCreatorReviewSourceFirst(outcome === "plan");
          setStage(outcome === "session" ? "study-now" : "plan-creator");
        }}
      />}
...
export function LoadingAccount({ inviteOnly = false }: { inviteOnly?: boolean }) {
  return <main className="entry-shell entry-fallback-shell">
    <header className="entry-nav"><BrandMark /><Link className="button ghost" href="/study-profile">Free Study Profile</Link></header>
    <section className="entry-fallback-card" aria-labelledby="entry-fallback-title">
      ...
      <div className="entry-startup-status" role="status" aria-live="polite">
        <span className="button-spinner dark" aria-hidden="true" />
        <div><strong>Checking for your YOVA account</strong><span>Your saved work is not being changed.</span></div>
      </div>
      <noscript><p className="entry-no-script">The interactive YOVA planner needs JavaScript. The Study Profile, privacy information, terms, and support pages remain available.</p></noscript>
    </section>
    <section className="entry-fallback-steps" aria-labelledby="entry-fallback-how">
      <div><span className="step-label">HOW YOVA WORKS</span><h2 id="entry-fallback-how">From a goal to one clear next step.</h2></div>
      <ol><li>Name the goal and deadline.</li><li>Add materials only when they help.</li><li>Follow a method YOVA explains.</li></ol>
    </section>
...
function OnboardingIntro({ onStart }: { onStart: () => void }) {
  return <main className="centered-shell"><BrandMark /><section className="setup-card"><span className="step-label">SET UP YOUR YOVA</span><h1>Make YOVA fit how you actually study.</h1><p>Eleven short, optional questions create the same study profile you can review later in You. About two minutes.</p><div className="info-strip"><Sparkles size={20} /><span>This records changeable preferences, not a brain type. Task requirements and checked work still decide what is valid.</span></div><button className="button primary large full" onClick={onStart}>Personalize YOVA <ArrowRight size={18} /></button></section></main>;
}
...
export function OnboardingQuestion({ index, answer, onAnswer, onNext, onBack }: { index: number; answer?: string; onAnswer: (answer: string) => void; onNext: () => void; onBack: () => void }) {
  const question = CANONICAL_PROFILE_QUESTIONS[index];
  const selectedId = question.options.some((option) => option.id === answer)
    ? answer
    : null;
...
function EarlySessionDialog({ plan, session, pending, issue, onCancel, onStart }: { plan: LearningPlan; session: LearningPlanSession; pending: boolean; issue: string | null; onCancel: () => void; onStart: (shiftRemainingPlan: boolean) => void }) {
  ...
  return <AccessibleModalDialog className="early-session-backdrop" labelledBy="early-session-title" describedBy="early-session-description" dismissible={!pending} onDismiss={onCancel}><section className="early-session-dialog"><span className="early-session-icon"><CalendarDays size={22} /></span><span className="step-label">YOU ARE AHEAD OF SCHEDULE</span><h2 id="early-session-title">Start {session.title} now?</h2><p id="early-session-description">This session is planned for {formatAgendaTime(session.scheduledFor)}. You can move forward now without skipping any unfinished content.</p><div className="early-schedule-choice"><Sparkles size={18} /><div><strong>Recommended: pull the calendar forward</strong><p>YOVA will move this session to now and shift the remaining {Math.max(0, unfinishedCount - 1)} {unfinishedCount - 1 === 1 ? "session" : "sessions"} by the same amount. The learning order and spacing stay intact.</p></div></div>{issue && <div className="chat-error"><AlertCircle size={16} /><span>{issue}</span></div>}<div className="early-session-actions"><button className="button ghost" data-modal-initial-focus disabled={pending} onClick={onCancel}>Cancel</button><button className="button secondary" disabled={pending} onClick={() => onStart(false)}>Start now, keep dates</button><button className="button primary" disabled={pending} onClick={() => onStart(true)}>{pending ? <span className="button-spinner" /> : <CalendarDays size={16} />} Start and adjust calendar</button></div></section></AccessibleModalDialog>;
}
...
function HomeScreen({ account, answers, plans, plan, sessionCompletions, sessionInterruptions, activeSessionCheckpoints, allowance, allowanceChecking, tutorQuestion, onTutorQuestion, onOpenTutor, onOpenYou, onStart, onOpenPlan, onCreatePlan, onStudyNow, milestones, onOpenAgenda }: { account: PreviewAccount | null; answers: string[]; plans: LearningPlan[]; plan: LearningPlan | null; sessionCompletions: SessionCompletion[]; sessionInterruptions: SessionInterruption[]; activeSessionCheckpoints: ActiveSessionCheckpoint[]; allowance: GuidedSessionAllowanceDisplayState; allowanceChecking: boolean; tutorQuestion: string; onTutorQuestion: (question: string) => void; onOpenTutor: () => void; onOpenYou: () => void; onStart: (planId?: string) => void; onOpenPlan: (planId: string) => void; onCreatePlan: () => void; onStudyNow: () => void; milestones: DeadlineMilestone[]; onOpenAgenda: () => void }) {
  ...
  const resumePoint = startDecision?.resumePoint ?? null;
  const displayedSessionHasSavedWork = startDecision?.canStartWithoutGeneration ?? false;
  const displayedStartBlocked = guidedSessionAllowanceBlocksNewStart(
    allowance,
    displayedSessionHasSavedWork,
    allowanceChecking,
  );
  const newStudyNowBlocked = guidedSessionAllowanceBlocksNewStart(allowance, false, allowanceChecking);
  ...
  const homeTotalMinutes = homeRoute?.timing.elapsedMinutes
    ...
    ?? null;
  const methodFit = homeRoute?.explanation.shortReason
    ?? readySession?.adaptationNote?.explanation
    ?? readySession?.methodReason
    ?? null;
      ...
      >
        <div className="hg-upnext-slide" key={displayedPlan.id} aria-live="polite">
          ...
          <div className="hg-chip-row">
            <span className="yv-chip navy">{homeSessionType}</span>
            <span className="yv-chip blue">{homeMethod}</span>
            <span className="yv-chip neutral">{homeTotalMinutes} minutes</span>
          </div>
          {methodFit && <p className="hg-why"><strong>WHY THIS · </strong>{methodFit}</p>}
        ...
        <div className="hg-upnext-foot">
          ...
          {recommendations.length > 6 ? <span className="hg-dots"><button type="button" className="hg-arrow" aria-label="Show previous recommendation" onClick={showPreviousRecommendation}><ChevronLeft size={16} /></button><span className="hg-dots-count">{recommendationIndex + 1} of {recommendations.length}</span><button type="button" className="hg-arrow" aria-label="Show next recommendation" onClick={showNextRecommendation}><ChevronRight size={16} /></button></span> : recommendations.length > 1 ? <span className="hg-dots">{recommendations.map((item, index) => <button key={item.id} type="button" className={index === recommendationIndex ? "active" : ""} aria-label={`Show recommendation ${index + 1} of ${recommendations.length}`} aria-pressed={index === recommendationIndex} onClick={() => setSelectedRecommendationId(item.id)} />)}<span className="hg-dots-count">{recommendationIndex + 1} of {recommendations.length}</span></span> : <span className="hg-dots-count">{completedCount} of {displayedPlan.sessions.length} sessions complete</span>}
        </div>
      </section>
      <section className="yv-card hero hg-today">
        <div className="hg-card-head"><h3 className="yv-serif-heading small">Today</h3><button type="button" className="hg-link" onClick={onOpenAgenda}>Calendar →</button></div>
    ...
    {openMilestones.length > 0 && <>
      ...
      <div className="hg-milestones">
        {nextMilestone && <section className="yv-tint hg-milestone">
          ...
          <button className="yv-pill outline small" onClick={onOpenAgenda}>View in calendar</button>
        </section>}
...
function LearningScreen({ revisionClient, plans, detailPlanId, sessionCompletions, sessionInterruptions, activeSessionCheckpoints, preferredMethodIds: selectedPreferredMethodIds, syncedPreferenceKey, statedPreferencesEnabled, onPreferredMethodIdsChange, onOpenPlan, onClosePlan, onStart, onCreatePlan, onArchiveStateChange, onDeletePlan, onAdjustPlan, onKnowledgeMapUpdate, onAttachMaterials }: { revisionClient: RevisionClient; plans: LearningPlan[]; detailPlanId: string | null; sessionCompletions: SessionCompletion[]; sessionInterruptions: SessionInterruption[]; activeSessionCheckpoints: ActiveSessionCheckpoint[]; preferredMethodIds: readonly CoreMethodId[]; syncedPreferenceKey: string | null; statedPreferencesEnabled: boolean; onPreferredMethodIdsChange: (methodIds: CoreMethodId[]) => void | Promise<void>; onOpenPlan: (planId: string) => void; onClosePlan: () => void; onStart: (planId: string) => void; onCreatePlan: () => void; onArchiveStateChange: (planId: string, action: "archive" | "restore") => Promise<LearningPlan["status"]>; onDeletePlan: (planId: string) => Promise<void>; onAdjustPlan: (input: PlanAdjustmentRequest) => Promise<void>; onKnowledgeMapUpdate: (planId: string, knowledgeMap: PlanKnowledgeMap) => void; onAttachMaterials: (planId: string, materialIds: string[]) => Promise<void> }) {
  const [view, setView] = useState<LearningSection>(() => {
    ...
    if (requestedPlan && (
      requestedPlan.creationIntent === "study_now"
      || canPresentPlanAsCompleted(requestedPlan)
    )) return "recent";
    return "active";
  });
...
function LearningOverview({ plans, allPlans, view, interruptions, activeSessionCheckpoints, onOpenPlan, onStart }: { plans: LearningPlan[]; allPlans: LearningPlan[]; view: "active" | "recent" | "archive"; interruptions: SessionInterruption[]; activeSessionCheckpoints: ActiveSessionCheckpoint[]; onOpenPlan: (planId: string) => void; onStart: (planId: string) => void }) {
  ...
  return <>
    ...
    <section className="learning-card-grid">{plans.map((plan) => {
      ...
      const resumePoint = readySession ? sessionStartRecoveryDecision({
        plan,
        session: readySession,
        interruptions: interruptions.filter((interruption) => interruption.planId === plan.id),
        restorableCheckpoints: activeSessionCheckpoints.filter((checkpoint) => checkpoint.planId === plan.id),
      }).resumePoint : null;
      ...
      return <article className="learning-goal-card" key={plan.id}>
        ...
        <div className="learning-card-copy"><h2>{plan.title}</h2><p>{topicDisplayLabel(plan.topic)}</p></div>
...
function LearningPlanDetail({ revisionClient, plan, view, completions, interruptions, activeSessionCheckpoints, changingStatus, onBack, onStart, onArchiveStateChange, onDeletePlan, onAdjustPlan, onKnowledgeMapUpdate, onAttachMaterials }: { revisionClient: RevisionClient; plan: LearningPlan; view: "active" | "recent" | "archive"; completions: SessionCompletion[]; interruptions: SessionInterruption[]; activeSessionCheckpoints: ActiveSessionCheckpoint[]; changingStatus: boolean; onBack: () => void; onStart: () => void; onArchiveStateChange: (action: "archive" | "restore") => void; onDeletePlan: () => Promise<void>; onAdjustPlan: (input: PlanAdjustmentRequest) => Promise<void>; onKnowledgeMapUpdate: (planId: string, knowledgeMap: PlanKnowledgeMap) => void; onAttachMaterials: (planId: string, materialIds: string[]) => Promise<void> }) {
  ...
  const presentAsCompleted = canPresentPlanAsCompleted(plan);
  const operational = isOperationalPlan(plan);
  const canManagePlan = operational && view !== "archive";
  const hasAdjustableUnfinishedWork = plan.sessions.some((session) => (
    (session.status === "ready" || session.status === "upcoming")
    && !isScheduledRetrievalSession(session)
  ));
  const readySession = plan.sessions.find((session) => session.status === "ready");
  ...
  return <>
    ...
    {canManagePlan && revisionRequest && <LivingPlanRevision key={revisionRequest.key} plan={plan} client={revisionClient} initialDelta={revisionRequest.delta} initialTopicId={revisionRequest.topicId} onClose={() => setRevisionRequest(null)} />}
...
function TutorHistoryDialog({
  ...
  loading: boolean;
  error: string | null;
  threads: TutorThreadSummary[];
  currentThreadId: string | null;
  onClose: () => void;
  onOpenThread: (thread: TutorThreadSummary) => Promise<void>;
}) {
  return <AccessibleModalDialog
    className="tutor-history-modal"
    labelledBy="tutor-history-title"
    onDismiss={onClose}
  >
    <aside className="tutor-history-panel">
      <header>
        <div><span className="step-label">ASK YOVA</span><h2 id="tutor-history-title">Previous chats</h2></div>
        <button data-modal-initial-focus aria-label="Close conversation history" onClick={onClose}><X size={19} /></button>
      </header>
      {loading
        ? <div className="chat-loading"><span className="button-spinner dark" /> Loading chats…</div>
        : error
          ? <div className="chat-error"><AlertCircle size={16} /><span>{error}</span></div>
          : threads.length === 0
            ? <div className="tutor-history-empty"><History size={21} /><strong>No saved chats yet</strong><p>Your finished Ask YOVA conversations will appear here.</p></div>
            : <div className="tutor-history-list">{threads.map((thread) => <button key={thread.id} className={thread.id === currentThreadId ? "selected" : ""} onClick={() => void onOpenThread(thread)}><span>{thread.contextTitle ?? "General"}</span><strong>{thread.title}</strong><small>{formatTutorThreadDate(thread.updatedAt)}</small></button>)}</div>}
    </aside>
...
function YouScreen({ account, answers, plans, sessionCompletions, sessionInterruptions, passwordAccountsEnabled, turnstileSiteKey, signingOut, onAnswersChange, onDisplayNameChange, onSignOut, onReset }: { account: PreviewAccount | null; answers: string[]; plans: LearningPlan[]; sessionCompletions: SessionCompletion[]; sessionInterruptions: SessionInterruption[]; passwordAccountsEnabled: boolean; turnstileSiteKey: string | null; signingOut: boolean; onAnswersChange: (answers: string[]) => void; onDisplayNameChange: (displayName: string) => Promise<void>; onSignOut: (options?: { accountAlreadyDeleted?: boolean }) => Promise<void>; onReset: () => Promise<void> }) {
  ...
  const canonicalProfile = canonicalLearnerProfileFromAnswers(answers);
  const canonicalState = consolidatePersonalizationStateForCanonicalV1(
    readPersonalizationStateFromAnswers(answers),
  );
  const isCloudAccount = account?.identityMode === "supabase";
  ...
  const confirmDataReset = async () => {
    ...
    try {
      ...
      setResetError(error instanceof Error
        ? error.message
        : "YOVA could not reset your learning data.");
      setResetting(false);
    }
  };
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        description="Keep one optional study profile, inspect comparable method evidence, and correct either whenever context is missing."
      />
      <section className="personalization-principle">
        <Sparkles size={20} />
        <div>
          <strong>Your profile is a changeable preference record, not a brain type.</strong>
          <p>
            Task, stage, time, and checked evidence decide which methods are valid.
            Your profile can only rank valid options and shape delivery.
          </p>
        </div>
      ...
      <CanonicalProfileCenter
        ...
        onEnabledChange={(enabled) => onAnswersChange(
          updatePersonalizationStateInAnswers(answers, (state) => (
            setPersonalizationControl(
              consolidatePersonalizationStateForCanonicalV1(state),
              "selfReport",
              enabled,
      ...
      <div className="you-grid">
        ...
        {account && (
          <AccountSecurityCard
            ...
            onPrepareDataExport={isCloudAccount
              ? (signal) => prepareAccountDataExport(account.id, { signal })
              : undefined}
            onDisplayNameChange={onDisplayNameChange}
            onSignOut={onSignOut}
          />
        )}
        <section className="section-block alpha-data-card">
          <div>
            <h3>{isCloudAccount ? "Cloud learning data" : "Private-alpha data"}</h3>
            <p>
              {isCloudAccount
                ? "Remove your study profile, plans, tutor conversations, results, and private uploaded materials. Your login identity will remain available."
                : "Reset the account, study profile, plans, and session results stored in this browser."}
            </p>
          </div>
          {confirmReset ? (
            <div className="reset-confirm">
              ...
              <span>
                {isCloudAccount
                  ? "YOVA will permanently remove your cloud learning data and uploaded files."
                  : "Only this browser’s private-alpha data will be removed."}
              </span>
            ...
            >
              <Trash2 size={16} />
              {" "}
              {isCloudAccount ? "Reset learning data" : "Reset private-alpha data"}
            </button>
          )}
        </section>
      </div>
...
export function SessionGenerationRecovery({ plan, session, briefing, coverage, failureState, issue, canStartMethod, onExit, onOpenGoal, onStartMethod, onRetry, onReviewSetup }: {
  ...
  return <main className="centered-shell session-recovery-shell">
    ...
    <section className={`plan-error-state session-recovery-state ${quotaClass}`} role={fallbackState.kind === "allowance_exhausted" ? "status" : "alert"}>
      ...
      {canStartMethod && <p className="session-method-alternative-note">A topic-scoped study-method guide is available below as ungraded practice. It does not replace a subject-specific lesson or count as proof of mastery.</p>}
...
function StandaloneStudyMethodSession({ plan, session, briefing, coverage, elapsedSeconds, progress, recoveryNotice, onProgressChange, onComplete, onExit }: {
  plan: LearningPlan | null;
  session: LearningPlanSession | null;
  briefing: SessionMethodBriefing | null;
  coverage: SessionCoverage | null;
...
type GuidedSessionProps = {
  ...
  methodWorkProgress: MethodWorkProgress;
  activityProgress: SessionActivityProgress | null;
  onMethodWorkProgressChange: (progress: MethodWorkProgress) => void;
  onActivityProgressChange: (progress: SessionActivityProgress) => void;
...
function GuidedSession({ plan, planSessionId, steps, step, selectedAnswer, outcome, attemptCount, confidence, priorConfidenceCaptured, answerRevealed, elapsedSeconds, capacityMinutes, rationale, coverage, methodBriefing, deliveryPolicy, supportPlan, sourceGrounding, workspaceClassName, issue, recoveryNotice, recoveryIssue, methodWorkProgress, activityProgress, onMethodWorkProgressChange, onActivityProgressChange, onCompleteMethodPractice, analyticsEnabled, browserPreviewMode, streamedLessons, onOpenStreamedLesson, onSkipStreamedLesson, onSelect, onEvaluate, onRetry, onConfidence, onReveal, onExit, onRedirectPlan, onNext }: GuidedSessionProps) {
  ...
  const scheduledReviewEvidenceCaptured = quickScheduledReview
    && step === steps.length - 1
    && outcome !== undefined;
  const scheduledReviewLearningSupportLocked = quickScheduledReview && !scheduledReviewEvidenceCaptured;
  const outsideMethodSession = plan?.studyMode === "outside_yova" && currentSession && !quickScheduledReview
    ? {
      ...currentSession,
  ...
  const evaluationEvidenceDisposition = semanticEvaluationEvidenceDisposition({
    evaluation: answerEvaluation,
    evaluationFailed: Boolean(answerEvaluationIssue),
  });
  const semanticEvaluationHasNoEvidence = answerRevealed
    && evaluationEvidenceDisposition === "no_evidence";
  const freeResponseReady = content.type !== "free_response" || answerRevealed;
  const retrievalRoundComplete = content.methodRuntime?.kind === "retrieval_round"
    && retrievalRoundActivityProgressIsComplete({
      progress: activityProgress,
      activityIndex: persistedActivityIndex,
      promptCount: content.methodRuntime.prompts.length,
    });
  const canContinue = content.methodRuntime?.kind === "retrieval_round"
    ? retrievalRoundComplete && teachingComplete
    : (!isQuestion || outcome !== undefined || semanticEvaluationHasNoEvidence)
      && teachingComplete
      && freeResponseReady;
  ...
  const freeResponseMode = content.type === "free_response"
    ...
    : "explanation";
  const conceptMapDraftComplete = content.methodRuntime?.kind !== "concept_map"
    || conceptMapDraftIsComplete(content.methodRuntime, selectedAnswer);
  ...
  const checkFreeResponse = async () => {
    if (!selectedAnswer?.trim() || !conceptMapDraftComplete || answerEvaluationPending) return;
  ...
  return <main className={`session-shell ${workspaceClassName}`}>
    ...
    <section className="session-content">
      <SessionGuidePanel session={currentSession} capacityMinutes={capacityMinutes} coverage={coverage} steps={steps} step={step} methodBriefing={methodBriefing} deliveryPolicy={deliveryPolicy} supportPlan={supportPlan} sourceGrounding={sourceGrounding} rationale={rationale} scheduledReviewEvidenceCaptured={scheduledReviewEvidenceCaptured} />
      <section className="session-workspace">
        ...
        {step === 0 && outsideMethodSession && outsideMethodBriefing && <StudyMethodPractice
          ...
          progress={methodWorkProgress}
          onProgressChange={onMethodWorkProgressChange}
        ...
        {content.methodRuntime?.kind === "retrieval_round"
          && <RetrievalRoundRuntime
            runtime={content.methodRuntime}
            activityIndex={persistedActivityIndex}
            initialProgress={activityProgress}
            onProgressChange={onActivityProgressChange}
          />}
        {content.methodRuntime?.kind === "worked_example" && <WorkedExampleRuntimePanel runtime={content.methodRuntime} />}
        {content.methodRuntime?.kind === "error_repair" && <ErrorRepairRuntimePanel runtime={content.methodRuntime} />}
        {content.methodRuntime?.kind !== "retrieval_round" && requiresConfidence && <ConfidenceCheck value={confidence} locked={outcome !== undefined || answerRevealed} onChange={onConfidence} />}
        {content.methodRuntime?.kind !== "retrieval_round" && content.type === "multiple_choice" && content.question && <div className="answer-grid">{content.question.map((answer) => {
      ...
      {content.methodRuntime?.kind !== "retrieval_round" && content.type === "free_response" && <div className="recall-response">
        ...
        {!answerRevealed && !awaitingRetryChoice ? <div className="recall-submit-actions">
          <button className="button secondary" disabled={!selectedAnswer?.trim() || !conceptMapDraftComplete || answerEvaluationPending || (requiresConfidence && !confidence)} onClick={() => void checkFreeResponse()}>
          ...
          {semanticEvaluationHasNoEvidence
            ? <div className="recall-actions"><span>YOVA did not record a correct or incorrect result from this check. Continue after comparing with the model answer.</span></div>
          ...
          <small className="privacy-note">{isImmediateRepair ? "This required recheck records whether the repaired concept now holds. A later review is added only if the pattern continues across sessions." : semanticEvaluationHasNoEvidence ? "Your typed answer is not saved, and this uncertain or unavailable check created no concept or method evidence." : answerEvaluation ? "Your answer was sent for a one-time AI check and is not saved. YOVA keeps only the concept result, confidence, and support level." : "Your typed answer is not saved. YOVA keeps only the concept result, confidence, and support level."}</small>
        </div>}
      </div>}
    ...
    {scheduledReviewLearningSupportLocked
      ? <aside className="session-tutor scheduled-review-support-locked" aria-label="Scheduled review help locked"><button className="session-tutor-launcher" type="button" aria-label="Ask YOVA is locked during this scheduled review" disabled><Target size={18} /><span>Help unlocks after all 3 answers</span></button></aside>
      : !isStreamedInstruction && <SessionTutor
        ...
        analyticsEnabled={analyticsEnabled}
      />}
    {reviewingModel && (reviewableTeaching || (reviewableStreamedLesson && reviewableStreamedLessonState)) && <AccessibleModalDialog className="session-model-review-backdrop" labelledBy="session-model-review-title" describedBy="session-model-review-description" onDismiss={() => setReviewingModel(false)}><section className="session-model-review-dialog"><header><div><span className="step-label">PREVIOUS LESSON</span><h2 id="session-model-review-title">Review the lesson, then return to the same question.</h2><p id="session-model-review-description">Your answer and session progress stay exactly where they are.</p></div><button className="button ghost" data-modal-initial-focus type="button" onClick={() => setReviewingModel(false)}><X size={17} /> Back to the question</button></header><div className="session-model-review-content">{reviewableStreamedLesson && reviewableStreamedLessonState ? <StreamedLessonReader state={reviewableStreamedLessonState} compact /> : reviewableTeaching ? <TeachingLessonCard teaching={reviewableTeaching} /> : null}</div><footer><button className="button primary" type="button" onClick={() => setReviewingModel(false)}><ArrowLeft size={17} /> Back to the question</button></footer></section></AccessibleModalDialog>}
    {changingDirection && <AccessibleModalDialog className="plan-direction-backdrop" labelledBy="plan-direction-title" describedBy="plan-direction-description" dismissible={!directionPending} onDismiss={() => { setChangingDirection(false); setDirectionIssue(null); }}><section className="plan-direction-dialog"><header><span className="plan-direction-icon"><Settings2 size={21} /></span><div><span className="step-label">CHANGE THE COURSE DIRECTION</span><h2 id="plan-direction-title">Tell YOVA what is off track.</h2><p id="plan-direction-description">Choose a supported adjustment to how you study the existing topics. Free-text topic changes are not available in an active plan. Completed work and learning evidence will stay.</p></div></header><label className={directionRequestLimit.isOverLimit ? "field-over-limit" : undefined}><span>What should be different?</span><textarea data-modal-initial-focus rows={5} value={directionRequest} disabled={directionPending} aria-invalid={directionRequestLimit.isOverLimit || undefined} aria-describedby="course-direction-limit" placeholder="Choose No calculations, Teach the basics first, or More real examples below." onChange={(event) => setDirectionRequest(event.target.value)} /><small id="course-direction-limit" className={`character-limit-feedback ${directionRequestLimit.isOverLimit ? "over-limit" : ""}`} role={directionRequestLimit.isOverLimit ? "alert" : undefined}>{formatCharacterLimit(directionRequestLimit)}</small></label><div className="plan-direction-examples"><button type="button" onClick={() => setDirectionRequest("Keep this conceptual. Do not include math or calculation exercises.")}>No calculations</button><button type="button" onClick={() => setDirectionRequest("Teach the foundations first, then use concrete examples before practice.")}>Teach the basics first</button><button type="button" onClick={() => setDirectionRequest("Use more real examples and case scenarios before independent work.")}>More real examples</button></div>{directionIssue && <div className="chat-error"><AlertCircle size={16} /><span>{directionIssue}</span></div>}<footer><button className="button ghost" disabled={directionPending} onClick={() => { setChangingDirection(false); setDirectionIssue(null); }}>Keep this plan</button><button className="button primary large" disabled={directionPending || directionRequest.trim().length < 5 || directionRequestLimit.isOverLimit} onClick={() => void redirectPlan()}>{directionPending ? <><span className="button-spinner" /> Rebuilding plan</> : <>Approve and rebuild <ArrowRight size={18} /></>}</button></footer></section></AccessibleModalDialog>}
...
function LessonAskAboutThis({ plan, planSessionId, activityIndex, activity, lessonContent, analyticsEnabled }: {
  ...
  return <aside className={`lesson-ask ${expanded ? "expanded" : ""}`}>
    {!expanded ? <button className="lesson-ask-launcher" type="button" onClick={() => setExpanded(true)}><MessageCircleMore size={17} /> Ask about this</button> : <section>
      ...
      <form onSubmit={(event) => { event.preventDefault(); void ask(); }}><textarea aria-label="Question about this lesson" rows={2} value={question} disabled={pending} placeholder="Ask for another explanation, example, connection, or clarification..." onChange={(event) => setQuestion(event.target.value)} /><button className="button primary" type="submit" disabled={pending || question.trim().length < 2}>{pending ? <span className="button-spinner" /> : <Send size={17} />} Send</button></form>
...
function SessionGuidePanel({ session, capacityMinutes, coverage, steps, step, methodBriefing, deliveryPolicy, supportPlan, sourceGrounding, rationale, scheduledReviewEvidenceCaptured }: { session: LearningPlanSession | null; capacityMinutes: number | null; coverage: SessionCoverage | null; steps: LessonStep[]; step: number; methodBriefing: SessionMethodBriefing | null; deliveryPolicy: SessionDeliveryPolicy | null; supportPlan: SessionSupportPlan | null; sourceGrounding: SessionSourceGrounding | null; rationale: string | null; scheduledReviewEvidenceCaptured: boolean }) {
  const quickScheduledReview = isScheduledRetrievalSession(session);
  const factualReviewDetailsLocked = quickScheduledReview && !scheduledReviewEvidenceCaptured;
  const focus = factualReviewDetailsLocked
    ? `Retrieve ${session?.reviewConcept ?? session?.title ?? "the scheduled idea"} without opening learning support.`
    : coverage?.focus ?? session?.objective ?? "Complete the next bounded learning objective.";
  const evidence = factualReviewDetailsLocked
    ? ["Answer all three questions before opening hints, explanations, or source details."]
    : coverage?.completionEvidence.length ? coverage.completionEvidence : session?.completionEvidence ?? ["Attempt the required check without hidden support."];
  ...
  const guide = <>
    ...
    {!factualReviewDetailsLocked && methodBriefing && <section className="session-method-playbook" aria-label={`How to use ${methodBriefing.name}`}><span>WHY THIS METHOD</span><p>{methodBriefing.why}</p><strong>Use it like this</strong><ol>{methodBriefing.how.slice(0, 3).map((instruction) => <li key={instruction}>{instruction}</li>)}</ol></section>}
    {!factualReviewDetailsLocked && adaptationReasons.length > 0 && <div className="session-personalization-proof"><Sparkles size={15} /><div><span>How YOVA adapted this method</span><ul>{adaptationReasons.map((reason) => <li key={reason}>{reason}</li>)}</ul></div></div>}
...
function SessionComplete({ currentSession, knowledgeMap, completionMode, completedAt, requiredContentCount, repairCount, elapsedSeconds, actualMinutes, correctAnswers, totalAnswers, observedGap, conceptEvidence, confidenceEvidence, nextSession, feedback, onFeedback, recoveryNotice, recoveryIssue, onFinish }: {
  ...
  const completionPreview: SessionCompletion = {
    ...
    actualMinutes,
    correctAnswers: completionMode === "unguided_practice" ? 0 : correctAnswers,
    totalAnswers: completionMode === "unguided_practice" ? 0 : totalAnswers,
    feedback,
    observedGap: completionMode === "unguided_practice"
      ? "Unguided practice completed; no topic evidence was recorded."
      : observedGap,
    completionMode,
    conceptEvidence: completionMode === "unguided_practice" ? [] : conceptEvidence,
    confidenceEvidence: completionMode === "unguided_practice" ? [] : confidenceEvidence,
  };
  ...
  const hasUnresolvedGap = conceptSummary.needsAnotherCheck.length > 0;
  const decision = currentSession && !sessionResourceHasDeferredPlanTargets(currentSession)
  ...
  const keepLabel = decision?.kind === "add_delayed_verification"
    ...
    : adaptationAgencyMode === "ill_customize"
      ...
      : "Keep current plan";
  const applyLabel = adaptationAgencyMode === "help_me_choose"
    ? "Confirm and update my plan"
    : adaptationAgencyMode === "ill_customize"
      ? "Use this recommendation"
      : "Update my plan";
  const decisionEyebrow = !hasRecommendedChange
    ? "NO CHANGE NEEDED"
    : adaptationAgencyMode === "yova_decides"
      ? "YOVA DECIDES · APPLIES ON FINISH"
      : adaptationAgencyMode === "help_me_choose"
        ? "HELP ME CHOOSE · CONFIRMATION NEEDED"
        : adaptationAgencyMode === "ill_customize"
          ? "I'LL CUSTOMIZE · RECOMMENDATION ONLY"
          : "YOVA RECOMMENDS";
  const agencyDecisionNote = decision?.kind !== "adapt_next_session"
    ? <><strong>Nothing changes until you approve it.</strong> YOVA will add only this delayed check; completed work and other learning targets stay in place.</>
    : adaptationAgencyMode === "yova_decides"
      ? <><strong>YOVA Decides will apply this supported change when you finish.</strong> Only the next session recipe changes; its target and later learning targets stay in place.</>
      : adaptationAgencyMode === "help_me_choose"
        ? <><strong>Nothing changes until you confirm this exact route.</strong> Your confirmation is bound to the displayed successor; a stale or different revision cannot be committed.</>
        : adaptationAgencyMode === "ill_customize"
          ? <><strong>Your selected route stays in place unless you choose this recommendation.</strong> YOVA will not silently replace it.</>
          : <><strong>Nothing changes until you approve it.</strong> YOVA will change only how the next session begins; later learning targets stay in place.</>;
  ...
  return <main className="centered-shell completion">
    ...
    <section className="completion-card">
      ...
      <PostSessionPersonalizationReceipt
        ...
        completion={completionPreview}
        decision={decision}
        adaptationAgencyMode={adaptationAgencyMode}
      />
...
function nextUnfinishedSessionAfter(
  sessions: LearningPlanSession[],
  currentSequence: number,
) {
  return sessions
    .filter((session) => (
      session.sequence > currentSequence
      && (session.status === "ready" || session.status === "upcoming")
    ))
