...
export function initialPlanBaselineMethod(selection:CanonicalMethodSelectionResult,answers:OnboardingAnswers,request:PlanGenerationRequest,session:LearningPlanSession):CanonicalMethodSelectionResult {
  ...
  return {...selection,selectedMethodId:baseline.methodId,selectedMethodName:baseline.methodName,authority:refs.length?"authorized_declaration":"task_baseline",changedFromBaseline:baseline.methodId!==selection.baselineMethodId,orderedMethodIds:[baseline.methodId,...selection.eligibleMethodIds.filter(id=>id!==baseline.methodId)],evidenceRefs:refs,learnerFacingReason:reason,ruleTrace:selection.ruleTrace.map(trace=>trace.ruleId===CANONICAL_METHOD_SELECTION_POLICY_VERSION?{...trace,result:`${refs.length?"authorized_declaration":"task_baseline"}:${baseline.methodId}`,reason,evidenceRefs:refs}:trace)};
}
