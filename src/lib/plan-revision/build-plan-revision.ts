...
export async function buildPlanRevision({ plan, request, delta, controls, protections, otherReservations, durationContext, methodContext, now, fill, contextKind, draftReceiptWindow }: {
  ...
  const after = { ...plan, revisionId, deadline: applied.request.deadline, knowledgeMap: nextMap, schedulePreferences: { timeZone: applied.request.timeZone, availability: applied.request.availability }, sessions };
