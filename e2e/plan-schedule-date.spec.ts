...
test("shorter sessions preserve weekly availability and explain insufficient time", async ({ page }) => {
  ...
  await panel.getByLabel("Change type").selectOption("set_availability");
  await panel.getByLabel("Future session length").selectOption("15");
  await panel.getByRole("button", { name: "Preview shorter sessions", exact: true }).click();
  await expect(panel.getByRole("button", { name: "Confirm changes" })).toBeEnabled();
  await panel.getByRole("button", { name: "Add another change", exact: true }).click();
  await panel.getByLabel("Change type").selectOption("set_deadline");
  await panel.getByLabel("Deadline", { exact: true }).fill(`${futureDate(1).input}T23:00`);
  await panel.getByRole("button", { name: "Preview change", exact: true }).click();
  await expect(panel).toContainText("does not fit before the deadline");
  await expect(panel.getByRole("button", { name: "Confirm changes" })).toBeDisabled();
  await expect(page.locator(".timeline-row")).toHaveCount(originalCount);
  await panel.getByRole("button", { name: "Add another change", exact: true }).click();
  await panel.getByLabel("Change type").selectOption("set_deadline");
  await panel.getByLabel("Deadline", { exact: true }).fill(`${futureDate(47).input}T23:00`);
  await panel.getByRole("button", { name: "Preview change", exact: true }).click();
