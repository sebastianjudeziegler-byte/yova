create table public.ai_usage_windows (
  ...
  action text not null check (action in ('plan_generation', 'session_generation', 'tutor_message')),
  window_kind text not null check (window_kind in ('minute', 'day')),
  window_started_at timestamptz not null,
  request_count integer not null check (request_count > 0),
  updated_at timestamptz not null default now(),
  primary key (user_id, action, window_kind, window_started_at)
);

create index ai_usage_windows_cleanup_idx
on public.ai_usage_windows(updated_at);
...
begin
  ...
  if request_action not in ('plan_generation', 'session_generation', 'tutor_message')
  ...
  if minute_count >= minute_limit then
    retry_after := greatest(retry_after, ceil(extract(epoch from (minute_start + interval '1 minute' - current_time)))::integer);
  end if;
  if day_count >= day_limit then
    retry_after := greatest(retry_after, ceil(extract(epoch from (day_start + interval '1 day' - current_time)))::integer);
  end if;
  ...
  insert into public.ai_usage_windows (
    ...
    current_user_id, request_action, 'minute', minute_start, 1, current_time
  )
  ...
  insert into public.ai_usage_windows (
    ...
    current_user_id, request_action, 'day', day_start, 1, current_time
  )
  on conflict (user_id, action, window_kind, window_started_at)
  do update set
    request_count = public.ai_usage_windows.request_count + 1,
  ...
  where user_id = current_user_id
    and updated_at < current_time - interval '8 days';
