-- Streamed lessons and optional teaching visuals use the same durable usage
-- gate as the existing plan, session, evaluation, and tutor calls. Keep the
-- table constraint and the security-definer function's allowlist in sync with
-- the server-side AIUsageAction union.
