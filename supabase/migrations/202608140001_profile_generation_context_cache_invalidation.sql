-- Personalization state includes learner-facing receipts and audit history.
-- Persist all of it, but invalidate unfinished generated lessons only when the
-- stable outer generation context changes. Existing schema-v3 profiles do not
-- have generationContext yet, so their first save may refresh once.
