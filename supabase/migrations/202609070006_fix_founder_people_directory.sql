-- Correct the founder directory created by 202609070005. PostgreSQL conditional
-- expressions such as coalesce, nullif, greatest, and least cannot be schema
-- qualified. Keep the already-applied migration immutable and replace the
-- function here with the same narrow output contract and corrected expressions.
...
declare
  ...
  normalized_search text := pg_catalog.lower(
    pg_catalog.btrim(coalesce(search_text, ''))
  );
  resolved_kind text := pg_catalog.lower(
    pg_catalog.btrim(coalesce(kind_filter, 'all'))
  );
  resolved_status text := pg_catalog.lower(
    pg_catalog.btrim(coalesce(status_filter, 'all'))
  );
  normalized_cursor_email text := pg_catalog.lower(
    pg_catalog.btrim(coalesce(cursor_email, ''))
  );
  resolved_limit integer := greatest(
    1,
    least(coalesce(result_limit, 25), 100)
  );
  result jsonb;
begin
  ...
  founder_emails as materialized (
    ...
    where nullif(pg_catalog.btrim(auth_user.email), '') is not null
  ),
  eligible_auth_users as materialized (
    ...
    where auth_user.deleted_at is null
      ...
      and nullif(pg_catalog.btrim(auth_user.email), '') is not null
  ...
  invite_rollup as materialized (
    select
      ...
      (
        pg_catalog.array_agg(
          nullif(pg_catalog.btrim(invite.display_name), '')
  ...
  people as materialized (
    select
      ...
      coalesce(account.display_name, invite.display_name) as display_name,
      ...
      coalesce(plan.plans_count, 0) as plans_count,
      coalesce(session.sessions_completed, 0) as sessions_completed,
      coalesce(session.study_minutes, 0) as study_minutes,
      lead.created_at as lead_created_at,
      lead.marketing_consent_at,
      case
        when lead.id is null then null
        when coalesce(response_count.report_count, 0) > 0
          then 'report_unlocked'
        else 'waitlist_only'
      end as profile_status,
      coalesce(response_count.report_count, 0) as report_count,
      ...
      case when lead.id is null then null else
        private.founder_safe_attribution_label(
          coalesce(response.utm_medium, confirmation.utm_medium),
          'none',
          100
        )
      end as medium,
      case when lead.id is null then null else
        private.founder_safe_attribution_label(
          coalesce(response.utm_campaign, confirmation.utm_campaign),
          'none',
          160
        )
      end as campaign,
      case when lead.id is null then null else
        private.founder_safe_attribution_label(
          coalesce(response.utm_content, confirmation.utm_content),
          'none',
          160
        )
      end as content,
      case when lead.id is null then null else
        private.founder_safe_attribution_label(
          coalesce(response.utm_term, confirmation.utm_term),
          'none',
          160
        )
      end as term,
      ...
      greatest(
        coalesce(account.account_created_at, '-infinity'::timestamptz),
        coalesce(account.last_sign_in_at, '-infinity'::timestamptz),
        coalesce(product.last_product_activity_at, '-infinity'::timestamptz),
        coalesce(invite.invite_updated_at, '-infinity'::timestamptz),
        coalesce(lead.updated_at, '-infinity'::timestamptz),
        coalesce(response_count.latest_report_at, '-infinity'::timestamptz),
        coalesce(report_view.report_viewed_at, '-infinity'::timestamptz),
        coalesce(confirmation.requested_at, '-infinity'::timestamptz),
        coalesce(lead.waitlist_joined_at, '-infinity'::timestamptz)
      ) as recent_at
  ...
  filtered_people as materialized (
    ...
    where (
      ...
      or pg_catalog.strpos(
        pg_catalog.lower(coalesce(person.display_name, '')),
        normalized_search
      ) > 0
    )
