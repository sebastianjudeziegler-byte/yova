-- Rebuild unfinished sessions from explicit content slices. Shorter time windows
-- create more sessions instead of relabeling the same amount of work.
...
begin
  ...
  if replacement_count < 1 or replacement_count > 14 then
    raise exception 'The replacement plan must contain between one and fourteen unfinished sessions.';
  end if;
  ...
  for replacement in select value from jsonb_array_elements(payload -> 'sessions')
  loop
