-- A required unguided-practice verification is always a fixed ten-minute
-- scheduled review. Its generator can cover at most two content targets and
-- two completion checks for one uniquely mapped topic in that window.
-- Enforce the same boundary before the completion transaction changes any
-- durable state, including for stale clients that
-- still offer unsupported method work.
...
begin
  ...
  if coalesce(jsonb_typeof(verification), 'null') <> 'object'
    ...
    or jsonb_array_length(verification -> 'topicIds') <> 1
    or coalesce(jsonb_typeof(verification -> 'contentTargets'), 'null') <> 'array'
    or jsonb_array_length(verification -> 'contentTargets') not between 1 and 2
    or coalesce(jsonb_typeof(verification -> 'completionEvidence'), 'null') <> 'array'
    or jsonb_array_length(verification -> 'completionEvidence') not between 1 and 2
    or jsonb_array_length(verification -> 'topicIds') <> (
      select count(distinct item.value #>> '{}')
      from jsonb_array_elements(verification -> 'topicIds') as item(value)
    )
    or jsonb_array_length(verification -> 'topicIds')
      > jsonb_array_length(verification -> 'contentTargets')
    ...
    raise exception 'The required guided verification is not valid or exceeds its ten-minute review capacity.';
  end if;
