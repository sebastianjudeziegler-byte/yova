...
declare
  ...
  declared_session_adjustment jsonb := payload -> 'sessionAdjustment';
  declared_adjustment_minutes numeric;
...
begin
  ...
  if declared_session_adjustment is not null
    and jsonb_typeof(declared_session_adjustment) <> 'null' then
    if jsonb_typeof(declared_session_adjustment) <> 'object' then
      raise exception 'Session adjustment data is not valid.';
    end if;

    if not (declared_session_adjustment ?& array['familiarity', 'availableMinutes', 'knownTargets', 'note'])
      or exists (
        select 1
        from jsonb_object_keys(declared_session_adjustment) as adjustment_keys(adjustment_key)
        where adjustment_key not in ('familiarity', 'availableMinutes', 'knownTargets', 'note')
      )
      or jsonb_typeof(declared_session_adjustment -> 'familiarity') <> 'string'
      or coalesce(declared_session_adjustment ->> 'familiarity', '') not in ('as_planned', 'already_know', 'need_teaching', 'challenge_me')
      or jsonb_typeof(declared_session_adjustment -> 'availableMinutes') not in ('number', 'null')
      or jsonb_typeof(declared_session_adjustment -> 'knownTargets') <> 'array'
      or jsonb_typeof(declared_session_adjustment -> 'note') <> 'string'
      or length(btrim(declared_session_adjustment ->> 'note')) > 500
      or octet_length(declared_session_adjustment::text) > 2500 then
      raise exception 'Session adjustment data is not valid.';
    end if;

    if jsonb_array_length(declared_session_adjustment -> 'knownTargets') > 4 then
      raise exception 'Session adjustment data is not valid.';
    end if;

    if exists (
      select 1
      from jsonb_array_elements(declared_session_adjustment -> 'knownTargets') as known_targets(known_target)
      where jsonb_typeof(known_target) <> 'string'
        or length(btrim(known_target #>> '{}')) not between 2 and 180
    ) then
      raise exception 'Session adjustment data is not valid.';
    end if;

    if jsonb_typeof(declared_session_adjustment -> 'availableMinutes') = 'number' then
      declared_adjustment_minutes := (declared_session_adjustment ->> 'availableMinutes')::numeric;
      if declared_adjustment_minutes <> trunc(declared_adjustment_minutes)
        or declared_adjustment_minutes not between 10 and 90 then
        raise exception 'Session adjustment data is not valid.';
      end if;
    end if;
  end if;
  ...
  insert into public.session_attempts (
    ...
    jsonb_build_object(
      ...
      'sessionAdjustment', declared_session_adjustment
    )
  )
