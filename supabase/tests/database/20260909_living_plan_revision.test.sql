...
select extensions.is(pg_temp.attempt_revision(jsonb_set(payload,'{operationId}','"b1000000-0000-4000-8000-000000000007"')),'PT409','an obsolete preview cannot overwrite the saved revision, and says so instead of reading as a retryable serialization failure') from revision_payload;
