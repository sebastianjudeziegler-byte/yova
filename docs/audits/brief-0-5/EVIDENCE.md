...
[Boundary green attempt 34382811713](https://github.com/sebastianjudeziegler-byte/yova/actions/runs/34382811713), `c2484b6`: all seven newly red cases now pass, as do the new malformed-equation/off-topic/duplicate negatives. The wider run is **90 pass / two stale fixture failures**, plus **12 grader passes**; live samples were skipped. Both old failures assumed `formula.outputs[0]` would be rejected and `outputs[1]` used. With the complete equation now accepted, its unchanged initial choices reach the learner. The positive now asserts those exact four initial choices, the same correct formula and a single provider call. The negative places identical/presentation-equivalent duplicates in **both** initial and recovery responses; it can no longer accidentally test an unused recovery. This changes no product protection. The [first green attempt](evidence/ci-boundary-first-green-c248/) retains the red expectation evidence; the next remote run must prove the corrected fixtures and all surrounding cases.

### Variable identity follow-up, before matcher edits

[Run 34382989996](https://github.com/sebastianjudeziegler-byte/yova/actions/runs/34382989996), `459b077`: **92/92 validator/eval and 12/12 grader checks pass**. A27 and the biology misconception each pass **3/3 live**. A15 is **FAIL / PASS / UNAVAILABLE**: sample 3 is a captured provider timeout, not semantic red. [Raw capture/result evidence](evidence/ci-boundary-green-459/).

Sample 1's initial response contains an incorrect partial answer (`f'g` where the question asks for first factor times derivative of second), so it must still be repaired. Its unchanged recovery is correct: the question defines `h(x)=u(x)v(x)` and the answer gives `h'(x)=u'(x)v(x)+u(x)v'(x)`. The equation matcher requires literal `f` and `g` from the plan target and rejects the structurally identical rule under renamed variables. This is a further representation issue at the same subject boundary. Add red for consistent renaming, then recognize only complete equivalent two-factor equations; a named result such as `h'` needs its explicit product definition from the question. Do not let a topic heading authorize an unrelated answer, conflate different arguments, accept missing/extra derivative terms, or accept an unbound/conflicting result variable. The permanent fixture retains the invalid first response and the valid recovery unchanged, requiring exactly two attempts.

[Renaming red 34383509950](https://github.com/sebastianjudeziegler-byte/yova/actions/runs/34383509950), `59b3679`: **four expected red / 31 passing boundary checks**, plus 12 passing grader checks ([artifact](evidence/ci-renamed-red-59/)). After that run, equation comparison recognizes the same complete product derivative under a consistent two-factor renaming. It retains exact term/prime structure, checks both independent variables, and accepts a named result only if the question explicitly binds that result to the same product. The compact-check boundary supplies only the question for this binding; subject proof still comes from the answer. Capture instrumentation forwards the new optional context unchanged. The invalid initial response remains in the fixture and must still trigger recovery. Green and fresh live samples are pending; no full-gate or deployment claim.

### Focused release green, complete gate restored

[Run 34383731300](https://github.com/sebastianjudeziegler-byte/yova/actions/runs/34383731300), `a7f3d7a`: **104/104 validator/eval checks and 12/12 grader checks pass**. **A15, A27 and the biology misconception each pass all three serial live samples**, with no retry or unavailable sample in this run. [Exact per-case results and provider captures](evidence/ci-renamed-green-a7/) and [artifact provenance](evidence/ci-renamed-green-a7/artifact-provenance.json) are retained. The exact renamed-variable fixture still repairs the incorrect original answer, and all malformed-equation, unbound-variable, wrong-term, mixed-argument, off-topic, deferred-topic and genuinely duplicate negatives pass.

The complete quality workflow is now restored: dependency/config audit, migration replay/database checks, full units, lint, TypeScript/build, desktop/mobile core and auth journeys, the phone comparison, and three serial full live runs. The three-sample release step temporarily adds only the pass-through audit hook to Vitest, saves provider responses in the artifact, and restores the original test configuration on exit. Product source, provider behavior and case assertions are unchanged by this hook; ordinary main/PR runs do not enable it. The full result remains pending. No local verification, merge, deployment, production settings change or Brief B repository implementation has occurred.

### Full gate `587f114`, attempt 1 unavailable; same-revision rerun pending

[Run 34384175894, attempt 1](https://github.com/sebastianjudeziegler-byte/yova/actions/runs/34384175894/attempts/1) tested merge `679fa3a49be4c782ecc879507414defaec99f7b5`. Audit/configuration and migration replay/database checks passed. Browser installation stopped on a **Google apt metadata hash mismatch** (expected SHA256 `233e56de019b57db89238fa7bcc3647718dbbea3a40c2dc1c633a8c8952aa9e9`, received `bc1428ab27c6d76ee9bb76de07f1ded0ddb4aaabd958fc72855634ef5894a4b3`). Integrity verification remained enabled. Unit, lint, build, core and auth steps were skipped; the phone comparison could not launch the absent Chromium executable on either source. These are unavailable checks, not product failures.

The incomplete attempt was cancelled and its [raw partial artifacts retained](evidence/ci-full-587-attempt1-unavailable/). Its full-live report incorrectly counts unfinished/pending observations as fail; **do not interpret its aggregate as 226 semantic failures**. Actual retained results are two completed passes (launch calculus and osmosis), one completed recall phase-order failure, and 225 pending/unrun observations. The recall reason matches the unchanged-main `main-recall-2` capture in run 34381102335; it is distinct from the earlier absent-reference observation. The runner reporting issue is backlogged. The rerun was requested only after cancellation completed: same run, attempt 2, job `102577671749`, same committed release revision. Full results remain pending. No local checks or integrity bypass were used.
