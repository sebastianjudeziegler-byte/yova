...
Integrated local result: 4,453 unit tests passed, 99 gated/skipped; 25 runner tests passed; lint and types passed. The one focused map recovery browser case passed, with a decoded/visually inspected replay indexed in RECORDINGS.md. Full browser/provider/migrated-DB evidence remains pending CI. Migrations: `20260917160001_topic_plan_workloads.sql` and `20260917170001_plan_session_authenticated_reads.sql`; readiness RPC v6 now requires contract `20260917170001` and `planSessionReads=true`.
...
CI409 exposed a real fresh-migration reload failure: plan_sessions had owner RLS but no authenticated SELECT grant. The second migration repairs reads without opening direct writes and makes readiness reject a missing grant or broadened owner policy. Local readiness checks pass; actual migrated replay remains pending. The earlier production completion timeout is still not causally explained by this finding.
