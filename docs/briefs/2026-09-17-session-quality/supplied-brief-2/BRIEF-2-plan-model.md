...
## Precondition - NOT yet satisfied. Do this first.

Plan revision must work on production end to end: change, confirm, receipt,
**Undo**, reload, confirm the undo held. Three defects have been found and
fixed on this path so far; a fourth is open.

Fixed and live: the missing migration (#89), the editedFields blank-vs-null
bug (#89), and Undo comparing timestamps as text (#94, eeb1ff9).

**Open:** Undo still refuses on production with "A changed session no longer
matches this preview." Cause found by Claude Code on 17 Sept: when a change
is built, `build-plan-revision.ts:204` omits the `editedFields` field when
it is empty; the database stores it as an empty list. Undo compares the
stored copy with the database, treats omitted and empty-list as different,
and refuses. CI missed it because the test plan's changed session never got
the empty list.

Fix it the same way as #94: its own branch, a test that reproduces the
failure against a real database (assert the fixture session carries the
empty list), a fix that treats omitted and empty-list as equal for this
field, CI, PR, founder merges. Then redo the production check on a fresh
topic. Only then start item 1.

Note: the throwaway plan "Water Cycle Quiz Foundations" has three topics
stuck as learned-elsewhere from failed undos. Use a different topic or a
fresh plan for the check.
