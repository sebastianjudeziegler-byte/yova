...
The three Markdown files from `/Users/sebastianziegler/Downloads/brief codex.zip` are preserved byte-for-byte in [supplied-brief-2](../briefs/2026-09-17-session-quality/supplied-brief-2/). Their requirements are inputs to this combined plan. The current user request is to refine our own issue brief and then execute it with those files.
