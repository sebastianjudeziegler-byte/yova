...
The audited inventory and quarantine are in `scripts/live-gate/policy.json`. Its 75 required case identities preserve the original 64 unit cases and two browser journeys, plus the nine Brief 0.5 cross-subject grader canaries; discovery also includes future live cases automatically. New failures are never automatically quarantined. To stabilize a flaky case later, retain the canary, attach repeated full-run evidence, and remove its FLAKY entry through a reviewed change. Brief 0 does not repair quarantined product defects.

For baseline classification, any pass among the three runs makes a historically failing case FLAKY, including 3/3 recovery. With no passes, semantic failures are STALE only when supported by the existing contract; otherwise REAL. Cases with only transport/dependency failures are UNAVAILABLE. Mixed semantic and unavailable attempts retain both counts; unavailable attempts are not scored as failures. If one case hits a stale assertion and a separate unresolved product failure, it remains REAL, with both reasons recorded. If correcting a stale assertion exposes an independent product failure or an intent question, reclassify the case REAL while preserving its original three-run results and the earlier classification. A later successful retry does not automatically quarantine it.
